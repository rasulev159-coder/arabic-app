...
export function MemoryPage() {
  ...
  const flip = (id: string) => {
    ...
    if (a.letterCode === b.letterCode && a.kind !== b.kind) {
      ...
      setSelected([]);
      setLocked(false);
      if (newFound === LETTERS.length) {
        const dur = Math.round((Date.now()-startTime)/1000);
        save({ mode:'memory', score:LETTERS.length, totalQ:LETTERS.length, durationSec:dur, letterResults: LETTERS.map(l=>({letterCode:l.code,correct:true})) });
        setPhase('result');
      }
    } else {
      setTimeout(() => {
  ...
  if (phase === 'start') return (
    <div className="min-h-screen flex flex-col items-center justify-center gap-6 px-4 text-center">
      ...
      <Button size="lg" onClick={startSession}>{t('memory.title')}</Button>
    </div>
  );

  if (phase === 'result') return (
    <div className="min-h-screen flex flex-col items-center justify-center px-4">
      <SessionResult score={LETTERS.length} total={LETTERS.length} mode="memory"
        level={user?.level ?? 'beginner'} durationSec={elapsed} onRestart={startSession} />
    </div>
  );
  ...
  return (
    <div className="max-w-2xl mx-auto px-2 py-4">
      <div className="flex justify-between font-cinzel text-xs text-[#9a8a6a] mb-4 px-2">
        <span>{t('memory.pairs', { found, total: LETTERS.length })}</span>
...
export function ListenPage() {
  ...
  const next = (q: ArabicLetter[], r: typeof results) => {
    ...
    setTimeout(() => speak(getLetterName(card, lang)), 300);
  };
  ...
  const startSession = () => {
    ...
    setQueue(q.slice(1)); setCurrent(q[0]); setOptions(makeOpts(q[0])); setAnswered(null);
    setTimeout(() => speak(getLetterName(q[0], lang)), 400);
  };
  ...
  return (
    <div className="min-h-screen flex flex-col items-center px-4 py-10 gap-6">
      ...
      <div className="bg-gradient-to-br from-[#201808] to-[#140f05] border border-[#3a2d10]
        ...
        <p className="font-cinzel text-xs tracking-widest text-[#9a8a6a] uppercase">🔊 Название буквы</p>
        <button onClick={() => current && speak(getLetterName(current, lang))}
...
export function FindPage() {
  ...
  const [row, setRow]       = useState<ArabicLetter[]>([]);
  const [score, setScore]   = useState(0);
  ...
  const nextRound = (r: typeof results, s: number) => {
    ...
    const pool = shuffle(LETTERS).slice(0, 9);
    const hasTarget = pool.some(l=>l.code===t.code);
    const finalRow = hasTarget ? shuffle(pool) : shuffle([...pool.slice(0,8), t]);
    setTarget(t); setRow(finalRow); setAnswered(null); setRound(r.length);
  };
  ...
  const pick = (idx: number) => {
    ...
    const ok = row[idx].code === target.code;
    setAnswered(idx);
    ...
    <div className="min-h-screen flex flex-col items-center px-4 py-10 gap-8">
      ...
      {target && (
        <div className="text-center">
          ...
          <p className="font-scheherazade text-7xl text-gold-light drop-shadow-[0_0_30px_rgba(201,168,76,0.5)]">{target.code}</p>
          <p className="font-cinzel text-sm text-[#9a8a6a] mt-1">{getLetterName(target, lang)}</p>
        </div>
      )}
      <div className="grid grid-cols-3 gap-3 w-full max-w-xs" dir="rtl">
        {row.map((letter, i) => {
          const isA = answered === i; const isC = letter.code === target?.code;
          let cls = 'border-[rgba(201,168,76,0.12)] bg-[rgba(255,255,255,0.04)] text-gold-light';
